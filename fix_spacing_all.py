...
pattern = r"const mobileNormal = mobile\.filter\(m => m\.provider !== 'Mixx By Yas'\);\s*const mobileMixx = mobile\.filter\(m => m\.provider === 'Mixx By Yas'\);\s*if \(mobileNormal\.length > 0\) \{.*?(?=paymentString = paymentString\.trim\(\);)"

replacement = """const mobileNormal = mobile.filter(m => m.provider !== 'Mixx By Yas');
...
# To be safe, let's use a regex that matches from `const mobileNormal =` up to `paymentString = paymentString.trim();`
